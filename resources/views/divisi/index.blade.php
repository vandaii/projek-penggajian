<x-dashboard-layout>
    ...
    <div class="bg-white py-4 px-2 translate-y-10 rounded-xl shadow-md">
        <div class="mb-3">
            <h1 class="font-bold">List Divisi</h1>
        </div>
        ...
        <table class="w-full bg-white text-left table-auto">
            <tr class="">
                <th class="py-3 border-b-2 border-t-2">Nama Divisi</th>
                <th class="py-3 border-b-2 border-t-2">Deskripsi</th>
                <th class="py-3 border-b-2 border-t-2">Aksi</th>
            </tr>
            @forelse ($divisis as $divisi)
                <tr>
                    <td class="py-3 border-b-2">{{ $divisi->name }}</td>
                    <td class="py-3 border-b-2">{{ $divisi->deskripsi }}</td>
                    <td class="py-3 border-b-2 flex spa">
                        <a href="{{ route('divisi.show', $divisi->id) }}">
                        ...
                        <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                            action="{{ route('divisi.destroy', $divisi->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
