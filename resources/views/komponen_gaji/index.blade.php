<x-dashboard-layout>
    ...
    <div class="bg-white py-4 px-2 translate-y-10 rounded-xl shadow-md">
        <div class="mb-3">
            <h1 class="font-bold">Komponen Gaji</h1>
        </div>

        <div class="flex justify-end items-center">
            <a href="{{ route('komponen_gaji.create') }}"
        ...
        <table class="w-full bg-white text-left table-auto">
            <tr class="">
                ...
                <th class="py-3 border-b-2 border-t-2">Potongan</th>
                <th class="py-3 border-b-2 border-t-2">Gaji Bersih</th>
            ...
            @if (Auth::user()->role == 'admin')
                @foreach ($komponens as $komponen)
                    <tr>
                        <td class="py-3 border-b-2">{{ $komponen->user ? $komponen->user->name : 'N/A' }}</td>
                        <td class="py-3 border-b-2">
                            <p>Rp. {{ number_format($komponen->gaji_pokok, 0, ',', '.') }}</p>
                        </td>
                        <td class="py-3 border-b-2">
                            <p>Rp. {{ number_format($komponen->tunjangan, 0, ',', '.') }}</p>
                        </td>
                        <td class="py-3 border-b-2">
                            <p>Rp. {{ number_format($komponen->total_potongan, 0, ',', '.') }}</p>
                        </td>
                        <td class="py-3 border-b-2">
                            <p>Rp. {{ number_format($komponen->gaji_bersih, 0, ',', '.') }}</p>
                        </td>
                        <td class="py-3 border-b-2 flex">
                            <a href="{{ route('komponen_gaji.edit', $komponen->id ?? 0) }}">
                            ...
                            <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                action="{{ route('komponen_gaji.destroy', $komponen->id ?? 0) }}" method="POST">
                                @csrf
                                @method('DELETE')
